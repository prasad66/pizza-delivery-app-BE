...
router.route("/signup").post(async (req, res) => {
    ...
    const userfromDB = await GetAdmin(
    ...
    const userdata = await GetAdmin({ username: username });
    const token = jwt.sign({ id: userdata._id }, process.env.secret_key);
    // console.log(token);
    const user1 = await updateAdmin({
...
router.route("/login").post(async (req, res) => {
    ...
    // return
    const userfromDB = await GetAdmin({ username: username });
...
router.route("/forgotpassword").post(async (req, res) => {
    ...
    console.log(process.env.my_gmail, process.env.my_pass)

    const userfromDB = await GetAdmin({ email: email });
    // console.log(userfromDB);
    if (!userfromDB) {
...
router.route("/resetpassword").post(async (req, res) => {
    ...
    // console.log(token);
    const data = await GetAdmin({ token: token });
    // console.log(data);
    ...
    const hashedpassword = await genpassword(password);
    const user = await updateAdminPass({
        email: email,
        password: hashedpassword,
    });
...
//     const userdetail = await GetAdmin({ username: data.username });
...
router.route("/getadmin").get(async (req, res) => {
    ...
    const user = await GetAdmin({ token: token });
    res.send(user);
});
